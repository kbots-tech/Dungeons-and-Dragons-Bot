...
class Classes(interactions.Extension):
    ...
    async def _classes(self, ctx, name=None, level=None):
        ...
        try:
            ...
            if level:
                async with self.session.get(f'{BASE_URL}{url_list[name]}/levels') as resp:
                    data = json.loads(await resp.text())

                embeds = []
                while page < len(data):

                    embed = discord.Embed(title=name, description=f'Level {page + 1}', color=Color.red())
                    try:
                    ...
                    page += 1
                    for name,value,field in fields:
                        embed.add_field(name=name, value=value, inline=field)
                    embeds.append(embed)
                ...
                if not name:
                    ...
                    page2 = discord.Embed(title=name, description='Proficiencies', color=Color.red())
                    choices = ""
                    proficiencies = ""
                    ...
                    page3 = discord.Embed(title=name, description='Starting Equipment', color=Color.red())
                    fields = []
                    for f in equipment:
                    ...
                    page3 = discord.Embed(title=name, description='Spell Casting', color=Color.red())
                    fields = []
                    try:
                    ...
                    page4 = discord.Embed(title=name, description=f'Subclasses', color=Color.red())
                    page = 4
                    fields = []
